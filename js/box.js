...
var $MH={
	...
	fontlist: function (){
		...
		for(var i=0;i<a.video.length;i++){
			if(c>$MH.limit){break;}
			if(a.video[i].link && a.video[i].pic && a.video[i].name){
			img_li += '<p class="eb-ucenter-layer-movie g-clear js-ebucenter-movie">'+"<a href=\"" + a.video[i].link + "\" target=\"_self\" class=\"name\">" + a.video[i].name + "</a></p>"
				c++;
			}
		}
		img_li = img_li != "" ? img_li : '<p class="eb-ucenter-layer-title">无观看记录</p>';
		return '<li class="eb-ucenter-layer-item">' + img_li + "</li>";
	},
